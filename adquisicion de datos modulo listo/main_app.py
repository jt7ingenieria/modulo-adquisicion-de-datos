...
async def main() -> None:
    ...
    try:
        ...
        if Config.DATA_COLLECTION_MODE == 'historical':
            logger.info(f"Modo de recolección: Histórico. Período: {Config.START_DATE} a {Config.END_DATE}, Timeframe: {Config.TIMEFRAME}")
            
            # --- Obtención de Datos Históricos (OHLCV) ---
            if 'ohlcv' in Config.HISTORICAL_DATA_TYPES_TO_FETCH:
                ...
                logger.info("Descarga de datos OHLCV omitida según la configuración histórica.")

            # --- Obtención de Otros Tipos de Datos Históricos (Ticker y Order Book) ---
            if 'ticker' in Config.HISTORICAL_DATA_TYPES_TO_FETCH or 'orderbook' in Config.HISTORICAL_DATA_TYPES_TO_FETCH:
                logger.info("\n--- Demostración de Obtención y Guardado de Otros Tipos de Datos (Snapshot REST) ---")
                for symbol in Config.SYMBOLS_TO_FETCH:
                    # Obtener Ticker
                    if 'ticker' in Config.HISTORICAL_DATA_TYPES_TO_FETCH:
                        ...
                        logger.info(f"[{symbol}] Descarga de Ticker (Snapshot REST) omitida según la configuración.")
                    
                    # Obtener Order Book
                    if 'orderbook' in Config.HISTORICAL_DATA_TYPES_TO_FETCH:
                        ...
                        logger.info(f"[{symbol}] Descarga de Libro de Órdenes (Snapshot REST) omitida según la configuración.")
            else:
                logger.info("Descarga de Ticker y Libro de Órdenes (Snapshot REST) omitida según la configuración histórica.")

        elif Config.DATA_COLLECTION_MODE == 'realtime':
            logger.info(f"Modo de recolección: Tiempo Real. Duración: {Config.REALTIME_COLLECTION_DURATION_SECONDS} segundos.")
            
            realtime_tasks = []
            for symbol in Config.SYMBOLS_TO_FETCH:
                if 'trades' in Config.REALTIME_DATA_TYPES_TO_FETCH:
                    trades_save_path = Config._get_realtime_trades_filename(symbol)
                    realtime_tasks.append(fetcher.watch_trades(symbol, Config.REALTIME_COLLECTION_DURATION_SECONDS, trades_save_path))
                
                if 'orderbook' in Config.REALTIME_DATA_TYPES_TO_FETCH:
                    orderbook_save_path = Config._get_realtime_orderbook_filename(symbol)
                    # Puedes ajustar el límite para el order book en tiempo real si es necesario
                    realtime_tasks.append(fetcher.watch_order_book(symbol, Config.REALTIME_COLLECTION_DURATION_SECONDS, orderbook_save_path, limit=10))
            
            if realtime_tasks:
                await asyncio.gather(*realtime_tasks)
            else:
                logger.warning("No se especificaron tipos de datos en tiempo real para recolectar.")

        else:
            logger.error(f"Modo de recolección no válido: {Config.DATA_COLLECTION_MODE}. Use 'historical' o 'realtime'.")
